...
const blur = (id) => document.getElementById(id).dispatchEvent(new Event("blur"));
/** Fire an `input` event on a field. */
const input = (id) => document.getElementById(id).dispatchEvent(new Event("input"));
...
describe("ContactForm", () => {
    ...
    describe("name field validation", () => {
        ...
        it("shows length error when name is a single character", () => {
            const form = new ContactForm();
            form.init();
            document.getElementById("fieldName").value = "P";
            blur("fieldName");
            expect(errorText("nameError")).toBe("Name must be at least 2 characters.");
        });
    ...
    describe("email field validation", () => {
        ...
        it.each([
            ...
            const form = new ContactForm();
            form.init();
            document.getElementById("fieldEmail").value = value;
            blur("fieldEmail");
            expect(errorText("emailError")).toBe("Please enter a valid email address.");
        });
        it.each(["user@example.com", "pat.ksor+tag@subdomain.co.uk", "a@b.io"])('accepts "%s" as a valid email', (value) => {
            const form = new ContactForm();
            form.init();
            document.getElementById("fieldEmail").value =
                value;
    ...
    describe("message field validation", () => {
        ...
        it("shows length error when message is fewer than 10 characters", () => {
            ...
            document.getElementById("fieldMessage").value =
                "Short";
            blur("fieldMessage");
            expect(errorText("messageError")).toBe("Message must be at least 10 characters.");
        });
        it("clears the error when a valid message is entered", () => {
            const form = new ContactForm();
            form.init();
            const field = document.getElementById("fieldMessage");
            blur("fieldMessage");
    ...
    describe("has-error class", () => {
        it("adds has-error to a field when validation fails", () => {
            ...
            form.init();
            blur("fieldName");
            expect(document.getElementById("fieldName").classList.contains("has-error")).toBe(true);
        });
