...
const allVars = () => mockGsapTo.mock.calls.map((c) => c[1]);
// ── Tests ─────────────────────────────────────────────────────────────────────
describe("BlobAnimation", () => {
    ...
    it("float tweens contain positional or scale properties", () => {
        ...
        ba.init();
        const vars = allVars();
        const hasPositionalOrScale = vars.some((v) => "x" in v || "y" in v || "scale" in v);
    ...
    it("staggers each blob with an increasing delay", () => {
        ...
        const delayBlob0 = calls[0][1]["delay"] ?? 0;
        const delayBlob1 = calls[2][1]["delay"] ?? 0;
        const delayBlob2 = calls[4][1]["delay"] ?? 0;
