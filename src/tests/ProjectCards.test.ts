...
const buildDOM = (count = 3) => {
  document.body.innerHTML = Array.from(
    { length: count },
    (_, i) => `<article class="project-card" id="card${i}">card ${i}</article>`,
  ).join("\n");
};
...
describe("ProjectCards — entrance animation", () => {
  ...
  it("calls gsap.set with opacity:0 and y:60 on all cards", () => {
    ...
    const setVars = mockGsapSet.mock.calls[0][1] as Record<string, unknown>;
  ...
  it("calls ScrollTrigger.batch with the card elements", () => {
    ...
    const firstArg = mockBatch.mock.calls[0][0] as NodeListOf<HTMLElement>;
  ...
  it("registers an onEnter callback with ScrollTrigger.batch", () => {
    ...
    const batchConfig = mockBatch.mock.calls[0][1] as Record<string, unknown>;
  ...
  it("onEnter callback calls gsap.to with opacity:1 and y:0", () => {
    ...
    const batchConfig = mockBatch.mock.calls[0][1] as Record<string, unknown>;
    const onEnter = batchConfig["onEnter"] as (batch: HTMLElement[]) => void;

    const fakeBatch = [document.getElementById("card0")!];
    onEnter(fakeBatch);

    const vars = mockGsapTo.mock.calls[0][1] as Record<string, unknown>;
  ...
  it("does nothing when no .project-card elements exist", () => {
    ...
    if (setCalls > 0) {
      const target = mockGsapSet.mock.calls[0][0] as NodeList;
